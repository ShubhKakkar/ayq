  ...
  const handlePostCreation = async () => {
    ...
    const response = await fetch("/api/blog/create", {
      ...
      body: JSON.stringify({
        title,
        slug:slugData,
        category: category.split(/[\s,]+/).map(category => category.toLowerCase()),
        thumbnail,
        description,
      ...
      <div className="basis-1/2 h-[calc(100vh-80px)] overflow-y-scroll bg-gray-100 p-8 hide-overflow">
        <div className="bg-white shadow-lg p-4">
          ...
          >
            <div>
              <img
                src={thumbnail || "https://images.unsplash.com/photo-1655635949212-1d8f4f103ea1?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NXx8M2R8ZW58MHwwfDB8fA%3D%3D&auto=format&fit=crop&w=500&q=60"}
              ...
              <div className="flex mb-4 space-x-2">
                {category?.split(/[\s,]+/).map(category => category.toLowerCase())?.map((e, index) => {
                  return <a key={index}
              ...
              <a className="flex items-center text-gray-700" href="#">
                <div>
                  ...
                  <p className="text-sm text-orange-400 font-semibold">Jan 02 2021</p>
                </div>
              </a>
            </div>
